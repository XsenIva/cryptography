...
def prov_t_cond(p, a, b, m, f):
    ...
    if result == list():
        f.write("Проверка p^1 != 1  Не прошла" '\n')
    else: 
        f.write("Проверка p^1 != 1  Прошла" '\n')
    return result
...
def generate_points(p, N, r, points : set, f):
    while len(points) != N:
        ...
        f.write(f"Коэффициент A = {a_coff}" '\n')
        f.write(f"Сгенерированы следующие точки:"'\n')
        f.write(f"x = {x}, y = {y}"'\n')
...
def main():
  ...
  while True:
    ...
    for p, n_big, r in params:
        f.write("Выбраны параметры p, N, r = " + f"{p, n_big, r }"'\n')
        points = set()
        while len(points) != n_big:
            pair_p_a, points = generate_points(p, n_big, r, points, f)
            if pair_p_a != ((None, None), None):
                p_point, a_coff = pair_p_a
                if not check_infinite_point(n_big, *p_point, a_coff, p):
                    f.write(f"Проверка на бесконечность точки: не прошла "'\n')
                    continue
                q_big = scalarMultiply(int(n_big // r), *p_point, p, a_coff)
                f.write(f"Вычислена точка Q = " + f"{q_big[0]}" + f", {q_big[1]}"'\n')
                f.write(f"Кривая (P, A ,Q, r) = " + f"{p}, " + f"{a_coff}, " + f"{q_big}, " + f"{r}")
                fe.write(f"{p} " + f"{a_coff}")
                xs = [q_big[0]]
                ys = [q_big[1]]
